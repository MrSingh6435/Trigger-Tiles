...
public class PlayerController : MonoBehaviour
{
    ...
    [SerializeField] private float _maxFallSpeedVilocity = -20f;
    ...
    {
        Collider2D isGruounded = Physics2D.OverlapBox(_feetTransform.position, _groundCheck, 0f, _groundLayer);
        return isGruounded;
    }
    ...
    private void ExtraGravity()
    {
        if (_timeInAir > _gravityDelay)
        {
            ...
            if (_rigidBody.linearVelocity.y < _maxFallSpeedVilocity)
            {
                _rigidBody.linearVelocity = new Vector2(_rigidBody.linearVelocity.x, _maxFallSpeedVilocity);
            }
        }
    }
